...
export default function SessionList() {
    ...
    return (
        <div className="bg-canvas dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            <h3 className="text-lg font-bold text-ink dark:text-moonlight mb-4 text-serif">
                {t('home.activeSessions')}
            </h3>
            {sessions.length === 0 ? (
                <EmptyState
                    type="sessions"
                    title={t('sessions.emptyTitle', 'No Active Sessions')}
                    description={t('sessions.emptyDesc', 'It seems you are not logged in anywhere else. That is great for security!')}
                />
            ) : (
