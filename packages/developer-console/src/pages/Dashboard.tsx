...
export default function Dashboard() {
    ...
    // Theme-aware background - simplified
    ...
    return (
        <div className={`min-h-screen ${bgClass} p-4 md:p-8 transition-colors duration-200`}>
            <div className="mx-auto max-w-5xl space-y-6 md:space-y-8">
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className={`text-2xl md:text-3xl font-bold ${textPrimary}`}>{t('dashboard.title')}</h1>
                        <p className={`text-sm md:text-base ${textSecondary}`}>{t('dashboard.subtitle')}</p>
                    </div>
                    <div className="flex flex-wrap gap-2 md:gap-3 items-center w-full md:w-auto">
                        <ThemeSelector />
                        <LanguageSwitcher />
                        <div className="flex-1 md:flex-none"></div> {/* Spacer for mobile alignment */}
                        <Button variant="primary" size="sm" className="md:text-sm" onClick={() => setShowCreate(!showCreate)}>
                            <Plus className="mr-2 h-4 w-4" /> {t('dashboard.newApp')}
                        </Button>
                        <Button variant="outline" size="sm" className="md:text-sm" onClick={logout}>
                            <LogOut className="h-4 w-4 md:mr-2" /> <span className="hidden md:inline">{t('common.logout')}</span>
                        </Button>
                    </div>
                </header>
